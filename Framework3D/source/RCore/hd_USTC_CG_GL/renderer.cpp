...
void Hd_USTC_CG_Renderer::Render(HdRenderThread* renderThread)
{
    ...
    for (auto&& node : node_tree->nodes) {
        ...
        try_fill_info("render_time_code", render_param->time_code);
        try_fill_info("render_scene_usd", &(render_param->global_usd_stage));
        try_fill_info("render_speed_of_light", render_param->speed_of_light);
    }
