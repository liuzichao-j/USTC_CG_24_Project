...
MassSpring::MassSpring(const Eigen::MatrixXd& X, const EdgeSet& E, float speed_of_light_set)
{
    ...
    speed_of_light = speed_of_light_set;
}
...
void MassSpring::step()
{
    ...
    if (time_integrator == IMPLICIT_EULER) {
        ...
        for (int i = 0; i < n_vertices; i++) {
            if (!dirichlet_bc_mask[i]) {
                float gamma = 1.0 / std::sqrt(1.0 - vel.row(i).squaredNorm() / speed_of_light / speed_of_light);
                acceleration.row(i) = (force.row(i) - force.row(i).dot(vel.row(i)) * vel.row(i) / speed_of_light / speed_of_light) / gamma;
            }
        }
        ...
        for (int i = 0; i < n_vertices; i++) {
            ...
            if (vel.row(i).norm() > speed_of_light) {
                vel.row(i) = speed_of_light * vel.row(i).normalized();
            }
            X.row(i) += h * vel.row(i);
        }
    }
